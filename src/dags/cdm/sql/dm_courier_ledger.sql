...
with orders_sums as (select order_id,
     ...
     courier_order_sums as (select *,
                                   case
                                       when rate_avg < 4 then greatest(orders_total_sum * .05, orders_count * 100)
                                       when rate_avg < 4.5 then greatest(orders_total_sum * .07, orders_count * 150)
                                       when rate_avg < 4.9 then greatest(orders_total_sum * .08, orders_count * 175)
...
from courier_order_sums t
         join dds.dm_courier c on t.courier_id = c.id
order by 1, 3, 4
;